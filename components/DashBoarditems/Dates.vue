...
onMounted(async () => {
  try {
    ...
    const { data } = await useFetch("/api/config/alert", {
      method: "GET",
      query: { userId: currentUser.id },
    });

    currentAlert.value = data.value || [];
  } catch (error) {
    logger.error({ service: "auth", message: `[DashboardItemsDates]: ${error}` });
    currentAlert.value = [];
  }
});

const sortedAlerts = computed(() => {
  const alerts = currentAlert.value?.filter((obj) => obj.category !== "New STIG Library Available") || [];
