...
const selectAllFilter = defineModel("selectAllFilter");

const toggleSelectAll = () => {
  const newFilterValue = !selectAllFilter.value;
  selectAllFilter.value = newFilterValue;

  Object.keys(filters.value).forEach((key) => {
    filters.value[key] = newFilterValue;
  });
};

const toggleFilter = (key) => {
  filters.value[key] = !filters.value[key];
  if (Object.values(filters.value).every((val) => val)) {
    selectAllFilter.value = true;
  } else {
    selectAllFilter.value = false;
  }
};
</script>
