<template>
  <Listbox v-model="selectedItem">
    <!-- <ListboxLabel class="text-sm font-medium text-gray-800 dark:text-gray-300"> Select {{ model.label }} </ListboxLabel> -->
    ...
    <div class="relative">
      ...
      >
        <span v-if="selectedItem" class="block truncate">{{ selectedItem }}</span>
      ...
      >
        <!-- <ListboxOptions
          class="relative z-20 mt-1 max-h-60 w-40 overflow-auto rounded-md bg-white py-1 text-base text-sm shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-700"
        > -->
        <ListboxOptions
          class="absolute left-0 z-10 mt-1 max-h-60 w-36 overflow-auto rounded-md bg-white text-sm shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-700"
        >
