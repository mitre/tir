            ...
            if (assessment.Stig) {
              ...
              if (!(assessment.Stig.id in StigLookupTable)) {
                ...
                if (foundMatch) {
                  ...
                  missingStig = true;
                  missingStigs.push(
                    assessment.Stig.title +
                      " (v" +
                      assessment.Stig.version +
                      "r" +
                      assessment.Stig.stigRelease +
                      ")",
                  );
                }
              }
          ...
          try {
            ...
            for (const sys of boundary.Boundary.Systems) {
              ...
              for (let key in SystemAssessmentStigs[sys.id]) {
                console.log(
                  "Adding Stig",
                  StigReference[SystemAssessmentStigs[sys.id][key]].value.id,
                  "to system",
                  systemPush.id,
                );
            ...
            for (const assessment of boundary.Assessments) {
              if (assessment.succeededByAssessmentId == null) {
                ...
                newAssessment.succeededByAssessmentId = assessment.succeededByAssessmentId;
                ...
                if (assessment.succeededByAssessmentId == null) {
                  for (const ai of assessment.AssessmentItems) {
                    if (ai.StigDatumId != null) {
                      ...
                      if (
                        ai.StigDatum != null &&
                        ai.StigDatum.StigOverrides != null &&
            ...
            const existingBoundary = await Boundary.findOne({
              where: { name: boundary.Boundary.name },
            });
