...
export async function importChecklistV3(
  ...
  try {
    ...
    const targetDataFromCkl = cklbData.target_data;
    const setIfCurrentlyEmpty = (key, newValue) => {
      const currentValue = system.getDataValue(key);
      if (
        ...
        system.setDataValue(key, newValue);
      }
    };

    setIfCurrentlyEmpty("hostName", targetDataFromCkl.host_name);
    setIfCurrentlyEmpty("role", targetDataFromCkl?.role);
    setIfCurrentlyEmpty("assetType", targetDataFromCkl?.target_type);
    setIfCurrentlyEmpty("marking", targetDataFromCkl?.classification);
    setIfCurrentlyEmpty("hostIP", targetDataFromCkl?.ip_address);
    setIfCurrentlyEmpty("hostMAC", targetDataFromCkl?.mac_address);
    setIfCurrentlyEmpty("hostFQDN", targetDataFromCkl?.fqdn);
    setIfCurrentlyEmpty("targetComment", targetDataFromCkl?.comments);
    setIfCurrentlyEmpty("techArea", targetDataFromCkl?.technology_area);
    // setIfCurrentlyEmpty("targetKey", targetDataFromCkl?.TARGET_KEY);
    setIfCurrentlyEmpty("webOrDatabase", targetDataFromCkl?.is_web_database);
...
export async function importChecklist(
  ...
  let assetFromCkl = jsonObj.CHECKLIST.ASSET;
  ...
  try {
    ...
    setIfCurrentlyEmpty("hostName", assetFromCkl?.HOST_NAME);
    setIfCurrentlyEmpty("role", assetFromCkl?.ROLE);
    setIfCurrentlyEmpty("assetType", assetFromCkl?.ASSET_TYPE);
    setIfCurrentlyEmpty("marking", assetFromCkl?.MARKING);
    setIfCurrentlyEmpty("hostIP", assetFromCkl?.HOST_IP);
    setIfCurrentlyEmpty("hostMAC", assetFromCkl?.HOST_MAC);
    setIfCurrentlyEmpty("hostFQDN", assetFromCkl?.HOST_FQDN);
    setIfCurrentlyEmpty("targetComment", assetFromCkl?.TARGET_COMMENT);
    setIfCurrentlyEmpty("techArea", assetFromCkl?.TECH_AREA);
    setIfCurrentlyEmpty("targetKey", assetFromCkl?.TARGET_KEY);
    setIfCurrentlyEmpty("webOrDatabase", assetFromCkl?.WEB_OR_DATABASE);
