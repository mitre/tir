...
export async function generatePoam(
  ...
  function getStatusOverride(reportItem) {
    if (reportItem.statusOverride === "Open") {
      return "Open";
    } else if (reportItem.statusOverride === "Not_Reviewed") {
      return "Not_Reviewed";
    } else if (reportItem.statusOverride === "NotAFinding") {
      return "NotAFinding";
    } else if (reportItem.statusOverride === "Not_Applicable") {
      return "Not_Applicable";
    } else {
      return "Open"; // Default to "Open" if no match
    }
  }
  ...
  for (const result of vulnResults) {
    ...
    newRow[Columns.rawSeverity] += `${result.riskFactor}\n`;
    ...
    for (const reportItem of result.NessusReportItems) {
      // Check and set the highest status
      if (highestStatus === null || getStatusOverride(reportItem) === "Open") {
        highestStatus = "Open";
      } else if (highestStatus !== "Open" && getStatusOverride(reportItem) === "Not_Reviewed") {
        highestStatus = "Not_Reviewed";
      } else if (
        highestStatus !== "Open" &&
        highestStatus !== "Not_Reviewed" &&
        getStatusOverride(reportItem) === "NotAFinding"
      ) {
        highestStatus = "NotAFinding";
      } else if (
        highestStatus !== "Open" &&
        highestStatus !== "Not_Reviewed" &&
        highestStatus !== "NotAFinding" &&
        getStatusOverride(reportItem) === "Not_Applicable"
      ) {
        highestStatus = "Not_Applicable";
      }

      newRow[Columns.controlVulnDesc] += `${reportItem.NessusReport?.System.name}\n`;
    }
    newRow[Columns.status] += `${nessusToPoamStatus(highestStatus)}\n`;
    poamArray.push(newRow);
  }
