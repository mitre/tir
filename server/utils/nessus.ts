...
import { NessusPlugin_Boundary, SystemInterface } from "~/db/models";
...
export async function importNessus(xmlContent: string, nessusMatches: NessusMatch[]) {
  ...
  for (const host of jsonObj.NessusClientData_v2.Report[0].ReportHost) {
    ...
    if (systemMatch) {
      const system = (await System.findByPk(systemMatch.SystemId, {
        include: {
          model: Boundary,
        },
      })) as SystemInterface;
      ...
      function getTagValue(host: any, tagName: string): string | undefined {
        const tags = host?.HostProperties?.[0]?.tag;
      ...
      let netBiosName = getTagValue(host, "netbios-name");
      if (!netBiosName) {
        netBiosName = getTagValue(host, "hostname");
      }
      const hostIp = getTagValue(host, "host-ip");
      const hostMac = getTagValue(host, "mac-address");
      const hostFqdn = getTagValue(host, "host-fqdn");

      setIfCurrentlyEmpty("hostName", netBiosName);
      setIfCurrentlyEmpty("hostIP", hostIp);
      setIfCurrentlyEmpty("hostMAC", hostMac);
      setIfCurrentlyEmpty("hostFQDN", hostFqdn);
